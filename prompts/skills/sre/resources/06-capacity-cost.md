...
| 신호 | 판단 기준 |
|------|----------|
| **운영 로그** | 인시던트 조사에 실제로 거슬러 올라가는 기간. 대부분 그보다 훨씬 길게 잡혀 있다 |
| **감사 로그** | **비용으로 줄이지 않는다.** 보존 기간이 외부 요건으로 정해지고 변조 방지·접근 통제가 함께 요구된다 (L1의 감사 이벤트). 기간을 임의로 제시하지 말고 **해당 요건의 발행처 원문을 찾아 확인**한 뒤 근거 URL과 함께 적는다 |
| 메트릭 | 용량 계획·계절성 비교에 필요한 기간. 해상도를 낮춰 장기 보관하는 방법이 있다 |
| 트레이스 | 전량 단기 + 에러·느린 요청만 장기 |

**두 로그가 같은 스트림에 섞여 있으면 보존 기간을 줄이자는 제안을 하지 않는다.** 운영 로그를 줄이는 순간 감사 기록이 함께 잘린다 — 먼저 분리를 제안하고, 분리된 뒤에 운영 로그만 줄인다. 분리 여부는 L1이 판정한다.
